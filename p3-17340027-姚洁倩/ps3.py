...
def get_word_score(word, n):
    ...
    sum_of_letters = 0
    for w in word:
        sum_of_letters += SCRABBLE_LETTER_VALUES[w]
    component_two = 7*wordlen-3*(n-wordlen)
    if component_two < 1:
        component_two = 1
    return sum_of_letters*component_two
...
def is_valid_word(word, hand, word_list):
    ...
    """
    word = word.lower()
    word_seq = get_frequency_dict(word)
    if hand.get('*',0) > 0:
        wildcard = 1
    else:
        wildcard = 0
    for i in word_seq:
        if hand.get(i,0) < word_seq.get(i,0):
            return False
    index = word.find('*')
    if index == -1:
    ...
    while handlen > 0:
        ...
        # Otherwise (the input is not two exclamation points):
            ...
            if is_valid_word(user_word,hand,word_list):
                earn_score = get_word_score(user_word,handlen)
                total_score += earn_score
                print(user_word,'earn',earn_score,'points. Total:',total_score,'points')
